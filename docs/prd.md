...
| **样式方案** | **Tailwind CSS** | **原子化 CSS 引擎**。无需在 HTML 和 CSS 文件间频繁切换，通过组合 Utility Classes 即可快速构建响应式、高定制化的 UI 界面，同时有效避免样式冲突和 CSS 体积膨胀。 |
| **UI 组件库** | **Ant Design (v6)** | 业界公认的标准中后台组件库。其 `Table`、`Drawer`、`Dropdown` 等复杂组件高度成熟，能极大缩短基础页面的搭建时间，保证交互的专业性。 |
